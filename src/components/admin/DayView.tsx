...
const statusConfig: Record<string, { label: string; bgClass: string; textClass: string; showDoubleCheck?: boolean }> = {
  scheduled: { label: 'Enviado', bgClass: 'bg-card', textClass: 'text-muted-foreground' },
  pre_confirmed: { label: 'Pré-confirmado', bgClass: 'bg-amber-50', textClass: 'text-amber-600' },
  confirmed: { label: 'Confirmado', bgClass: 'bg-primary/5', textClass: 'text-primary', showDoubleCheck: true },
  waiting: { label: 'Em espera', bgClass: 'bg-yellow-50', textClass: 'text-yellow-700' },
  in_progress: { label: 'Em atendimento', bgClass: 'bg-orange-50', textClass: 'text-orange-700' },
  completed: { label: 'Concluída', bgClass: 'bg-muted/50', textClass: 'text-muted-foreground' },
  cancelled: { label: 'Cancelada', bgClass: 'bg-destructive/5', textClass: 'text-destructive' },
  no_show: { label: 'Faltou', bgClass: 'bg-destructive/5', textClass: 'text-destructive' },
};
    ...
    <div className="bg-card border border-border rounded-2xl overflow-hidden">
      {TIME_SLOTS.map((slot) => {
        ...
        return (
          ...
          >
            {/* Time column */}
            <div className="w-16 lg:w-20 shrink-0 flex items-start justify-end pr-4 py-4 text-muted-foreground text-sm font-medium">
              {slot}
            </div>
            ...
            <div className="flex-1 py-2 pr-4">
              {apt && status ? (
                <div
                  ...
                  className={cn(
                    'h-full flex items-center justify-between px-4 py-3 rounded-xl cursor-pointer transition-all hover:shadow-md border-l-4',
                    status.bgClass
                  )}
                  style={{
                ...
                >
                  <div className="min-w-0">
                    <p className="font-semibold text-foreground truncate">{patient?.name || 'Paciente'}</p>
